...
お知らせ本文にある開催期間の表記を読み取って、次の 3 つを行います。
ZZZ と FF14 の両方が対象です。

| ゲーム | 読み取る表記 |
| --- | --- |
| ZZZ（HoYoLAB） | `2026/08/24 11:00（JST） ～ 2026/09/07 04:59（JST）` |
| FF14（Lodestone） | `2026年8月19日（水）12:15頃 ～ 10月18日（日）23:59頃`（終了側の年は省略可） |
| FF14（時刻なし） | `応募期間 2026年8月7日（金）～ 8月31日（月）`（00:00 〜 23:59 とみなす） |
...
- 期間を読み取るのは、HoYoLAB のフィードと `"detect_event_period": true` を付けた
  フィードだけです。FF14 は `FF14公式 / Lodestone トピックス` にのみ付けてあります。
  `Lodestone お知らせ` はメンテナンス・障害情報の日時を拾ってしまうため対象外です
- 紅蓮祭・新生祭などのシーズナルイベントは、トピックス本文に
  「8月12日（水）より開催！」としか書かれておらず、期間は特設ページ側にあります。
  そこで `event_period_follow_link` にリンク先の URL 断片を並べておくと、本文中の
  リンクを 1 段だけ辿って期間を探します。FF14 では `["/lodestone/special/"]` を
  指定してあります。**この絞り込みは必須**で、公式の短縮 URL（`sqex.to`）は
  特設ページとは限らず別の記事に飛ぶことがあり、無条件に辿ると無関係な記事の
  期間を拾ってしまいます。一度調べた記事は `state.json` の `event_links_checked`
  に控えるので、毎回取りに行くことはありません
...
- `detect_event_period`: 記事本文から開催期間を読み取り、イベント告知・一覧・終了間近通知の
  対象にする（既定 `false`）。メンテナンスや障害情報の日時まで拾わないよう、
  イベント告知が流れるフィードにだけ付ける
- `event_period_follow_link`: 本文に期間が無いとき、リンクを 1 段辿って探す。
  採用してよいリンク先の URL 断片を配列で指定する（例: `["/lodestone/special/"]`）
